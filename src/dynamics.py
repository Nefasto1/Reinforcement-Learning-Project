...
def dynamic_step(F1_mod: float, F2_mod:float, F3_mod: float, 
        ...
        distance_shuttle_moon  = distance(com_0 * pixel_side, moon_coords * pixel_side)
        distance_shuttle_earth = distance(com_0 * pixel_side, earth_coords * pixel_side)

        F_grav_moon = G * mass_shuttle * mass_moon / distance_shuttle_moon**2
        beta = np.arccos((moon_coords - com_0)[1]*pixel_side/distance_shuttle_moon)
        F_grav_moon_x = F_grav_moon * np.sin(beta)
        F_grav_moon_y = F_grav_moon * np.cos(beta)
        
        F_grav_earth = G * mass_shuttle * mass_earth / distance_shuttle_earth**2
        gamma = np.arccos((earth_coords-com_0)[1]*pixel_side/distance_shuttle_earth)
        F_grav_earth_x = F_grav_earth * np.sin(gamma)
        F_grav_earth_y = F_grav_earth * np.cos(gamma)
